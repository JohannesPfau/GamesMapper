...
files = glob.glob("best_of_steam_sellers*.txt")
...
pd.merge(all_games,all_games,on = ['appid']).groupby(['year_x','year_y']).agg({'appid':'count'})['appid'].unstack(level=-1).style.background_gradient()

# COMMAND ----------

all_games_gdco.to_csv('bos_sellers_2019_2025.csv',index=False)
